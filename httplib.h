...
inline std::string file_extension(const std::string &path) {
    ...
    static auto re = boost::regex("\\.([a-zA-Z0-9]+)$");
  ...
  if (hints.ai_family == AF_UNIX) {
    ...
    if (sock != INVALID_SOCKET) {
        ...
        hints.ai_addrlen = static_cast<socklen_t>(
            ...
            sizeof(addr.sun_family) + std::strlen(addr.sun_path));  
...
find_content_type(const std::string &path,
    ...
    // First, we check if the extension is in the user_data map
    ...
    // Map of extensions for content types
    ...
    // Checks if the extension exists in the content_types map

    auto content_it = content_types.find(ext);

    if (content_it != content_types.end()) {
    ...
    // If not found, returns default content type
...
inline bool can_compress_content_type(const std::string &content_type) {
    ...
    // Maps known content types for compression.
    ...
    // Otherwise, if it is a 'text/' type, check if it is not 'text/event-stream'.

    return content_type.rfind("text/", 0) == 0 && content_type != "text/event-stream";
...
inline bool parse_range_header(const std::string &s, Ranges &ranges) try {
  ...
  static boost::regex re_first_range("bytes=(\\d*-\\d*(?:,\\s*\\d*-\\d*)*)");



  if (boost::regex_match(s, m, re_first_range)) {
      ...
      split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
          ...
          static boost::regex re_another_range("\\s*(\\d*)-(\\d*)");

          boost::cmatch cm;



          if (boost::regex_match(b, e, cm, re_another_range)) {
  ...
  if (res.has_header(auth_key)) {
      ...
      if (pos != std::string::npos) {
          ...
          if (type == "Basic") {
              ...
              auto beg = boost::sregex_iterator(s.begin(), s.end(), re);

              for (auto i = beg; i != boost::sregex_iterator(); ++i) {
...
inline std::string append_query_params(const std::string &path,
    ...
    const static boost::regex re("[^?]+\\?.*");

    auto delm = boost::regex_match(path, re) ? '&' : '?';
